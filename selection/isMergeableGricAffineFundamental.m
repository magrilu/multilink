function [ok, msScore, msOutput] = isMergeableGricAffineFundamental(X, L, i, j, lambda1 , lambda2,sigma)
...
mi = fit_fm_affine(Xi);
mj = fit_fm_affine(Xj);
mij = fit_fm_affine(Xij);
% compute the residuals
ri = res_fm_geo(Xi, mi);
rj = res_fm_geo(Xj, mj);
rij = res_fm_geo(Xij, mij);
...
ok = gAfter < gBefore;
%% package result
msScore.model = 'affine_fundamental';
